...
func TestHeaderViewShowsInternalTrackerBoard(t *testing.T) {
	...
	assert.Contains(t, out, ".aria/board")
}
...
func TestHeaderViewShowsRemoteTrackerScope(t *testing.T) {
	...
	h = h.Update(HeaderData{
		ModelName:    "gpt-5",
		TrackerType:  "github",
		TrackerScope: "https://github.com/ivanzzeth/symphony/aria/issues",
	})
	...
	assert.Contains(t, out, "junhoyeo/aria")
	assert.Contains(t, out, "URL:")
}

func loadTestImage(t *testing.T) image.Image {
	t.Helper()
	f, err := os.Open(filepath.Join("..", "..", ".github", "assets", "aria.png"))
