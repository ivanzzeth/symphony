...
import (
	...
	"github.com/ivanzzeth/symphony/aria/internal/agent"
	"github.com/ivanzzeth/symphony/aria/internal/team"
	"github.com/ivanzzeth/symphony/aria/internal/tmux"
...
func newIntegrationMockRunner(workspace string, processCount int) *integrationMockRunner {
	...
	handlers["tmux new-window -t aria-team-integration -n worker-1 -P -F #{pane_id}"] = func(_ []string) ([]byte, error) {
		return []byte("%1\n"), nil
	}
	...
	if processCount > 1 {
		handlers["tmux new-window -t aria-team-integration -n worker-2 -P -F #{pane_id}"] = func(_ []string) ([]byte, error) {
			return []byte("%2\n"), nil
		}
