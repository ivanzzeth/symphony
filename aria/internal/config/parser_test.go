	...
	tests := []struct {
	...
	}{
		{
			...
			assertionsFunc: func(t *testing.T, cfg *WorkflowConfig) {
				...
				require.NoError(t, err)
				assert.Equal(t, "https://linear.app/aria/project/aria-symphony-demo-4bded72b77d3", projectURL)
...
`
	...
	assert.Equal(t, ".aria/custom-board", cfg.LocalBoardDir())
