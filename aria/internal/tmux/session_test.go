...
func TestNewSession(t *testing.T) {
	...
	require.NotNil(t, s)
	assert.Equal(t, "aria-workers", s.Name)
...
func TestSessionCreate(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			results: map[string]mockResult{
				"tmux has-session -t aria-team": {err: errors.New("exit status 1")},
			},
			assertErr: func(t *testing.T, err error) {
				require.NoError(t, err)
			},
			callCount: 2,
			lastCall:  []string{"new-session", "-d", "-s", "aria-team"},
		},
		{
			name: "returns already exists error",
			results: map[string]mockResult{
				"tmux has-session -t aria-team": {},
			},
			assertErr: func(t *testing.T, err error) {
				require.Error(t, err)
			...
			callCount: 1,
			lastCall:  []string{"has-session", "-t", "aria-team"},
		},
		{
			name: "returns clear error when tmux is missing",
			results: map[string]mockResult{
				"tmux has-session -t aria-team": {err: errors.New("exit status 1")},
				"tmux new-session -d -s aria-team": {
...
func TestSessionListPanes(t *testing.T) {
	runner := &MockRunner{
		results: map[string]mockResult{
			"tmux list-panes -t aria-team -F " + paneListFormat: {
