...
func TestWorkerBootstrapBootstrap(t *testing.T) {
	runner := &MockRunner{
		results: map[string]mockResult{
			"tmux new-window -t aria-team -n worker-1 -P -F #{pane_id}": {output: []byte("%3\n")},
		},
	}
...
func TestWorkerBootstrapBootstrapValidationErrors(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			config:  BootstrapConfig{WorkerID: "", TeamName: "team", WorkDir: "/tmp/work", CLICommand: "aria"},
			errLike: "worker id is empty",
		},
		{
			name:    "empty team name",
			config:  BootstrapConfig{WorkerID: "worker-1", TeamName: "", WorkDir: "/tmp/work", CLICommand: "aria"},
			errLike: "team name is empty",
		},
		{
			name:    "empty work dir",
			config:  BootstrapConfig{WorkerID: "worker-1", TeamName: "team", WorkDir: "", CLICommand: "aria"},
...
func TestWorkerBootstrapBootstrapCleansUpOnFailure(t *testing.T) {
	runner := &MockRunner{
		results: map[string]mockResult{
			"tmux new-window -t aria-team -n worker-1 -P -F #{pane_id}": {output: []byte("%7")},
