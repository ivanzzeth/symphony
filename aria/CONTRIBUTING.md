...
Optional (needed only to run Aria end-to-end, not to build or test it):
...
```bash
# 1. Fork and clone
git clone https://github.com/<your-fork>/aria.git
cd aria
...
```
aria/
├── cmd/aria/          # CLI entry point (Cobra)
├── internal/
...
```bash
make build          # build dashboard SPA, then compile ./aria
...
```bash
go run ./cmd/aria --config testdata/workflow.demo.md
go run ./cmd/aria --config testdata/workflow.demo.md --port 8080  # with dashboard
go run ./cmd/aria --config testdata/workflow.demo.md --no-tui     # headless
```
...
If you have questions, feel free to open a [GitHub Discussion](https://github.com/junhoyeo/aria/discussions) or leave a comment on the relevant issue.
