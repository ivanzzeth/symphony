...
<img alt="Aria Logo" src="https://raw.githubusercontent.com/junhoyeo/aria/main/.github/assets/aria.png" width="300px" />
...
![Aria Demo (TUI in Action)](https://raw.githubusercontent.com/junhoyeo/aria/main/.github/assets/demo.png)

</div>

Aria is a terminal-first orchestrator for issue-driven agent runs, with an optional local web dashboard for live visibility.

## Current scope

Today Aria ships with:
...
```bash
brew install junhoyeo/aria/aria
```

### Download from GitHub Releases

Pre-built binaries for macOS and Linux (amd64/arm64) are available on the
[Releases](https://github.com/junhoyeo/aria/releases) page.

### Build from source

```bash
git clone https://github.com/junhoyeo/aria.git
cd aria
bun install
make build
```
...
> **Note:** `go install github.com/junhoyeo/aria/cmd/aria@latest` works for the
...
./aria --config testdata/workflow.demo.md
```
...
```bash
LINEAR_API_KEY=your-linear-token \
./aria --config testdata/workflow.demo.md --port 8080
```
...
```bash
LINEAR_API_KEY=your-linear-token \
./aria --config testdata/workflow.demo.md --no-tui
```
...
--log-file string    log output path (default "aria.log")
...
```text
aria team run --config workflow.md [flags]
...
- `WORKFLOW.md` is watched with `fsnotify`; on parse errors, Aria keeps the last known good config.
...
| `goroutine` | Workers run as goroutines within the aria process — lighter weight, no tmux dependency | |
...
Team state is persisted as JSON files under `.aria/state/team/{teamName}/`.

## Workflow file format

Aria reads a Markdown workflow file with YAML front matter followed by the prompt template body.
...
- Aria writes the rendered task prompt into `.aria/runner/<runner>/...` inside the workspace and instructs the team runtime to execute from that file.
...
- **Internal Board**
  - File-based local issue tracking under `.aria/board/` — no external service required
...
When `--port` is set, Aria serves the embedded dashboard and a small JSON/SSE API.
...
```bash
make build            # build dashboard, then build ./aria
...
```bash
go run ./cmd/aria --config testdata/workflow.demo.md --port 8080
```
...
|------|---------|-------------|---------|
| &nbsp;&nbsp; <img height="64px" src="https://raw.githubusercontent.com/junhoyeo/aria/main/.github/assets/charm/charm-bubbletea.webp" alt="Bubble Tea" /> | [**Bubble Tea**](https://github.com/charmbracelet/bubbletea) | `charm.land/bubbletea/v2` | TUI framework (Elm architecture) |
| <img height="64px" src="https://raw.githubusercontent.com/junhoyeo/aria/main/.github/assets/charm/charm-lipgloss.webp" alt="Lip Gloss" /> | [**Lip Gloss**](https://github.com/charmbracelet/lipgloss) | `charm.land/lipgloss/v2` | Styling & layout |
| <img height="64px" src="https://raw.githubusercontent.com/junhoyeo/aria/main/.github/assets/charm/charm-bubbles.webp" alt="Bubbles" /> | [**Bubbles**](https://github.com/charmbracelet/bubbles) | `charm.land/bubbles/v2` | Reusable TUI components |
| <img height="64px" src="https://raw.githubusercontent.com/junhoyeo/aria/main/.github/assets/charm/charm-log.webp" alt="Log" /> | [**Log**](https://github.com/charmbracelet/log) | `github.com/charmbracelet/log` | Structured logging |
...
[Homebrew tap](https://github.com/junhoyeo/homebrew-aria).
