...
import (
	...
	aria "github.com/ivanzzeth/symphony/aria"
...
func newRootCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Aria is a Go reimplementation of OpenAI's Symphony.
...
progress in a terminal UI built with the Charm stack.`,
	...
	cmd.Flags().StringVar(&logFile, "log-file", "aria.log", "log output path")
...
func run(cfgPath string, noTUI bool, logFile, logLevel string, dryRun bool, port int) error {
	...
	if port > 0 {
		...
		dashboardFS, err := fs.Sub(aria.DashboardDistFS, "packages/dashboard/dist")
		if err != nil {
