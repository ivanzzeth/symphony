...
func TestFlagDefaults(t *testing.T) {
	...
	tests := []struct {
		...
		{"log-file default", "log-file", "aria.log"},
...
`, boardDir))
	...
	err := run(cfgPath, true, filepath.Join(t.TempDir(), "aria.log"), "info", false, 0)
	require.NoError(t, err)
...
`, boardDir))
	...
	err := run(cfgPath, true, filepath.Join(t.TempDir(), "aria.log"), "info", true, 0)
	require.NoError(t, err)
}
