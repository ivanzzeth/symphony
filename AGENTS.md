...
Optional `symphony-*` skill directories mirror the canonical `pull`, `commit`, `push`, `land`, `linear`, `debug`, and `harness` skills for IDE aliases and older prompts; `symphony-harness` tracks `harness` content for the same reason.
...
| 2026-05-12 | WEB-128 harness reconfigure: workflow path fallback + AGENTS tree sync | harness + symphony-harness SKILL, symphony-dispatch, harness-agent, AGENTS.md | Injected `/tmp/.../WORKFLOW.md` may be absent locally; fall back to `elixir/WORKFLOW.md`; directory structure lists all `symphony-*` skill dirs |
