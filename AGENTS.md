...
| 2026-05-12 | WEB-101 harness reconfigure: WORKFLOW path fallback (`elixir/WORKFLOW.md` when `/tmp` injected path missing); canonical YAML `name` for `land`/`push`; `symphony-land` mirror sync; `symphony-push`/`symphony-pull`/`symphony-linear` bodies use `pull`/`commit`/`push`/`linear`; checklist + dispatch + harness Phase 0 alignment | `.agents/skills/{land,push,symphony-land,symphony-push,symphony-pull,symphony-linear,harness,symphony-harness}/**`, `harness-agent.md`, `AGENTS.md` | Linear WEB-101 |
