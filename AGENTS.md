...
- **Injected workflow path:** if dispatch names a path that cannot be read (often a stale `/tmp/...` copy) but `elixir/WORKFLOW.md` exists, read **`elixir/WORKFLOW.md`** as the contract and note the fallback in workpad **`Notes`** — still **never** edit the workflow file from harness or issue-execution work (see `.agents/skills/harness/references/symphony-dispatch.md`)
...
| 2026-05-12 | WEB-98 harness reconfigure | `.agents/` (harness + symphony-harness mirrors, harness-agent), AGENTS.md | Injected-path fallback to `elixir/WORKFLOW.md`; WORKFLOW Steps 0–4 mapping in issue-execution checklist; unattended Phase 0 audit summary; test scenario for missing injected path |
