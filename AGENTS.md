...
`symphony-*` directories mirror canonical skills (`harness`, `pull`, `land`, `linear`, `commit`, `debug`, …) for IDE aliases—treat them as equivalent to the canonical skill directories when present.
...
| 2026-05-12 | WEB-105 harness reconfiguration | harness + symphony-harness SKILL.md, symphony-dispatch.md (both), harness-agent.md, AGENTS.md directory tree | Injected workflow path may be missing on agent workspace (e.g. stale `/tmp/...`); document fallback to `elixir/WORKFLOW.md`, change-history + workpad Notes; expand AGENTS.md tree to list `symphony-*` alias dirs |
