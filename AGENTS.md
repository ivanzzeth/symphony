...
- **Harness / contract file path:** If dispatch context or ticket text cites a workflow file path that is **missing or unreadable** (for example a stale `/tmp/...` snapshot), read **`elixir/WORKFLOW.md`** as the authoritative contract for harness alignment and record intended path, outcome, and file read in **`## Codex Workpad` → `Notes`**.
...
| 2026-05-12 | WEB-87 harness reconfigure: injected-path fallback + checklist step mapping | `.agents/skills/harness/SKILL.md`, `symphony-dispatch.md`, `harness-agent.md`, `issue-execution-checklist.md`, `AGENTS.md` | Ticket `/tmp/...` workflow snapshot unavailable; document monorepo fallback to `elixir/WORKFLOW.md` |
