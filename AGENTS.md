...
**Execution Rules:**
- **Contract file:** YAML + Markdown execution contract lives in **`elixir/WORKFLOW.md`**. Symphony may inject a different readable path for a given run (for example a workspace copy under `/tmp/.../WORKFLOW.md`); treat that file as authoritative **when readable**. If the injected path is missing or unreadable, fall back to **`elixir/WORKFLOW.md`** (see `.agents/skills/harness/references/symphony-dispatch.md` → *Contract file resolution*). Never edit the workflow contract from harness work — only `.agents/` and `AGENTS.md`.
...
- **WORKFLOW Contract (Markdown template):** (1) unattended — never ask the human for follow-up actions; final message = completed actions + blockers only (no “next steps for user”); if blocked early, record details in the workpad and move the issue per WORKFLOW (blocked-access / state rules); (2) workpad-first; (3) reproduce first; (4) single workpad; (5) ticket metadata current, no issue-body planning; (6) scope discipline → separate Backlog follow-ups with `related` / `blockedBy` when needed
...
- **Prerequisite (WORKFLOW):** Linear MCP or `linear_graphql` must be available for unattended orchestrator-driven runs. If absent, **do not** ask the user to configure it mid-run; follow the **blocked-access escape hatch** — record what is missing, why it blocks acceptance/validation, and exact human unblock actions in the workpad (and one blocker comment if no workpad yet per Guardrails), then move per WORKFLOW blocked-access / state rules. **Operator recovery:** an authenticated **`linear` CLI** in the shell may perform the same transitions and workpad edits when GraphQL is unavailable (see `.agents/skills/linear/SKILL.md`); this does not waive the prerequisite for Symphony’s own polling session. GitHub is not a default blocker (try fallbacks first, document in workpad)
- **Blocked-access brief vs unattended tone (WORKFLOW):** Contract point 1 is “no follow-up actions” / blockers-only final message; the **Blocked-access escape hatch** is the deliberate exception — keep unblock guidance **action-oriented and in the workpad only** (not a separate “please configure X” chat to the human mid-run)
...
- **Workpad acceptance shape (WORKFLOW Step 1):** for **user-facing** changes, include a **UI walkthrough** acceptance criterion (end-to-end path to validate); for **app-touching** changes, add explicit **app-specific flow checks** in `Acceptance Criteria` (launch path, changed interaction, expected result), not only generic test commands
...
| 2026-05-12 | WEB-86 harness reconfigure: injected-path fallback, CLI recovery, Step 0–4 checklist map | AGENTS.md, harness-agent.md, harness/SKILL.md, linear/SKILL.md, symphony-dispatch.md, issue-execution-checklist.md | Contract file resolution when injected `WORKFLOW.md` unreadable; `linear` CLI documented as operator fallback; checklist step-alignment note vs `elixir/WORKFLOW.md` |
