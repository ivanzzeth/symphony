...
- **Prerequisite (WORKFLOW):** Linear MCP or `linear_graphql` must be available for unattended orchestrator-driven runs. If absent, **do not** ask the user to configure it mid-run; **stop** and follow the **blocked-access escape hatch** — record what is missing, why it blocks acceptance/validation, and exact human unblock actions in the workpad (and one blocker comment if no workpad yet per Guardrails), then move per WORKFLOW blocked-access / state rules. **Operator recovery:** an authenticated **`linear` CLI** in the shell may perform the same transitions and workpad edits when GraphQL is unavailable (see `.agents/skills/linear/SKILL.md`); this does not waive the prerequisite for Symphony’s own polling session.
- **Blocked-access escape hatch (WORKFLOW):** GitHub is **not** a default blocker — try fallbacks (alternate remote/auth mode), document in workpad. **Non-GitHub** missing tool/auth: move to **`In Review`** with a blocker brief in the workpad (what is missing, why it blocks, exact unblock action). Contract point 1 is “no follow-up actions” / blockers-only final message; this escape hatch is the deliberate exception — keep the brief **action-oriented** and **in the workpad only** (not a separate “please configure X” chat to the human mid-run).
