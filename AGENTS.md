...
| 2026-05-12 | Harness sync (WEB-100): injected workflow path fallback + mirror parity | harness/SKILL.md, symphony-dispatch.md, issue-execution-checklist.md, harness-agent.md; mirrored under `symphony-harness/` | Ticket dispatch path `/tmp/.../WORKFLOW.md` unavailable in agent env — documented read-only fallback to `elixir/WORKFLOW.md`; aligned `harness/` and `symphony-harness/` trees; `symphony-linear` / `symphony-land` pointers in checklists |
