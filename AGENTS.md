...
**Execution Rules:**
- **WORKFLOW prompt steps (mirror `elixir/WORKFLOW.md` Markdown):** **Step 0 — Route** (fetch state, apply **State → Skill** table, `Backlog` → stop; `Todo` → `In Progress` → `## Codex Workpad` before substantive work; CLOSED/MERGED PR → fresh branch from `origin/develop`; inconsistencies → workpad `Notes` + safest branch). **Step 1 — Workpad bootstrap** (environment stamp; `Plan` / `Acceptance Criteria` / `Validation` / `Notes`; mirror ticket validation as required; **`pull`** with evidence in `Notes` toward `origin/{{ workspace.base_branch }}` / `develop`). **Step 2 — Execute** (implement; revert proof edits; **`commit`** → **`push`**; **`land sweep`**; `In Review` only when **Completion bar** met). **Step 3 — In Review** (no implementation; no ticket-content planning edits; poll). **Step 4 — Rework** (close PR; delete workpad; fresh branch; full redo).
...
| 2026-05-12 | WEB-107 continuation (attempt #1): `harness` SKILL parity with `symphony-harness` | `.agents/skills/harness/SKILL.md`, AGENTS.md | Matched continuation scenario + References checklist line to `symphony-harness` mirror |
