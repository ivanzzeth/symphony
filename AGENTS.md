...
- **Step 0 state/content mismatch:** if workflow state and issue content or attachments disagree, document it in workpad **`Notes`** and follow the safest **Step 0: Route** branch (WORKFLOW **Step 0: Route**)
- **`In Review` freeze:** no implementation work; do **not** change **ticket content** (WORKFLOW **Step 3: In Review**); wait and poll for review/approval outcomes per WORKFLOW **Step 3**
...
- **Plan hygiene:** never leave completed work unchecked in the workpad (WORKFLOW **Step 2: Execute**)
...
- **Step 0 branch hygiene:** if a PR for the current branch is `CLOSED` or `MERGED` on GitHub, do not reuse that branch or prior implementation state—branch fresh from `origin/develop` and restart kickoff (WORKFLOW **Step 0: Route**)
- Workpad top: one fenced `text` environment stamp `<hostname>:<abs-path>@<short-sha>` (WORKFLOW **Step 1: Workpad bootstrap**); omit issue metadata redundant with Linear fields
...
- After each mandated sync via **`pull`** (WORKFLOW **Step 1: Workpad bootstrap**), record **`pull skill evidence`** in the workpad **`Notes`** (merge source(s), `clean` vs `conflicts resolved`, resulting `HEAD` short SHA)—see `pull` skill
...
- **Kickoff sync gate (WORKFLOW Step 1: Workpad bootstrap):** before substantive implementation, confirm **`pull`** has run, **`pull` skill evidence** is recorded in workpad **`Notes`**, and repo state (`branch`, `git status`, `HEAD`) is understood
...
- **Workpad acceptance shape (WORKFLOW Step 1: Workpad bootstrap):** add `Plan`, `Acceptance Criteria`, `Validation`, `Notes` with checklists; mirror ticket `Validation` / `Test Plan` / `Testing` into the workpad as **required** items; for **app-touching** changes, add explicit **app-specific flow checks** in `Acceptance Criteria`, not only generic test commands
- **Ticket validation gate (WORKFLOW Step 2: Execute):** when present, execute **all** ticket-provided `Validation` / `Test Plan` / `Testing` requirements; **unmet items mean incomplete work** (no partial credit). Copy those sections into workpad `Acceptance Criteria` and `Validation` as **required** checkboxes—**no optional downgrade** (WORKFLOW **Step 1: Workpad bootstrap**)
- **Before `In Review` (WORKFLOW Step 2: Execute + Completion bar):** poll PR feedback and checks; read **Manual QA Plan** on the PR when present; re-open and refresh the workpad so `Plan`, `Acceptance Criteria`, and `Validation` **exactly** match completed work; every required ticket-provided validation item must be explicitly marked complete in the workpad
- App-touching changes: follow WORKFLOW **Step 2: Execute** and the **Completion bar before In Review**, including runtime validation and media where required (WORKFLOW: “runtime validation + media captured”; typical tooling includes `launch-app` and `github-pr-media` when the change touches app files or behavior)
...
| 2026-05-12 | Harness sync to WORKFLOW (explicit Step 0–4 labels, injected workflow path) | AGENTS.md, `harness` + `symphony-harness` references (`issue-execution-checklist.md`, `symphony-dispatch.md`) | `elixir/WORKFLOW.md` prompt uses **Step 0: Route** … **Step 4: Rework**; align checklist + dispatch with numbered steps; Phase 0 reads injected path then falls back to `elixir/WORKFLOW.md` |
| 2026-05-12 | Harness continuation (WEB-102 retry #1): verification vs `elixir/WORKFLOW.md`; injected workflow path unavailable | `symphony-harness/SKILL.md`, AGENTS.md | Injected `/tmp/symphony-elixir-harness-82370/WORKFLOW.md` not present locally — audited against canonical `elixir/WORKFLOW.md`; references dirs already matched; byte-synced alias `symphony-harness/SKILL.md` with `harness/SKILL.md`; `git pull origin develop` clean |
