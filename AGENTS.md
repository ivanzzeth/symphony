...
**Execution Rules:**
- **WORKFLOW prompt shape:** the injected `elixir/WORKFLOW.md` template uses **Step 0** Route through **Step 4** Rework, plus **Completion bar**, **Guardrails**, **Blocked-access escape hatch**, and **Prerequisite**; harness docs mirror that structure. Legacy sub-step labels in old change-history rows (for example “Step 2.13”) refer to prior contract versions and are not authoritative.
...
| 2026-05-12 | Harness sync to WORKFLOW (WEB-115): Steps 0–4 section map, injected-path fallback, checklist/dispatch wording | AGENTS.md, `harness` + `symphony-harness` skills, `harness-agent.md` | Align `.agents/` with current `elixir/WORKFLOW.md` contract; ticket’s temp workflow path unavailable — used repo canonical file |
