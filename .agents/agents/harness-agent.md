...
9. **Injected workflow path fallback** — Prefer the dispatch-supplied workflow path when readable. If it is missing or unreadable, record the reason in workpad **`Notes`** and use **`elixir/WORKFLOW.md`** as the authoritative contract for harness verification and `.agents/` sync in this monorepo.
