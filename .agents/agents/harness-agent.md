...
- **Missing / unreadable workflow contract:** Report and exit if neither the
  dispatch-injected workflow file (when present) nor fallback **`elixir/WORKFLOW.md`**
  can be read. See `symphony-dispatch.md` → *Injected path resolution*. Do not
  guess the contract from stale ticket text alone.
