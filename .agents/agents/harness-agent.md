...
- **Missing or unreadable workflow at injected path**: Try `elixir/WORKFLOW.md`
  in the workspace root (Symphony monorepo canonical contract). If that file is
  also missing or unreadable, report and exit—cannot configure harness without
  a readable execution contract.
