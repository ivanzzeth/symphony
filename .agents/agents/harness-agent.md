...
- **Missing or unreadable contract file**: If the dispatch-injected path is absent or empty, read **`elixir/WORKFLOW.md`** (canonical for this monorepo) and continue. Only stop if **both** the injected path and `elixir/WORKFLOW.md` are unavailable. Document the path resolution in the issue **`## Codex Workpad` → `Notes`**.
