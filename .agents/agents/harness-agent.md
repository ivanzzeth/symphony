...
- **Missing WORKFLOW.md**: Report and exit; cannot configure harness without the
  ...
  `elixir/WORKFLOW.md` (path passed to the running orchestrator). If the
  orchestrator injected a temp path that is gone, fall back to `elixir/WORKFLOW.md`
  per `symphony-dispatch.md` (*Fallback when the injected path is missing*).
