...
- **Missing or unreadable workflow contract**: If the **injected** dispatch path
  is missing or unreadable, fall back to **`elixir/WORKFLOW.md`** (canonical
  monorepo contract) per `symphony-dispatch.md` → *Contract file resolution*. If
  **both** are unavailable, report and exit — cannot configure harness without
  the Symphony execution contract.
