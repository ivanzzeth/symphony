...
- **Missing or unreadable injected workflow path**: If the dispatch context path
  cannot be read, fall back to the repository canonical contract at
  **`elixir/WORKFLOW.md`**. If that file is also missing, report and exit; cannot
  configure harness without the Symphony execution contract.
