...
- **Missing or unreadable workflow file**: Try the injected dispatch path first; if it fails, fall back to **`elixir/WORKFLOW.md`** in this monorepo, document the fallback in the workpad or AGENTS.md **Change History**, then continue. **Only** report and stop if **both** are unreadable. Never edit the workflow file from harness work.
