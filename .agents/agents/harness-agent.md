...
- **Missing WORKFLOW.md**: Report and exit; cannot configure harness without the
  ...
  `elixir/WORKFLOW.md` (path passed to the running orchestrator). If dispatch
  references a **temporary or stale absolute path** (for example a `/tmp/.../WORKFLOW.md`
  snapshot) that is no longer readable, **fall back to `elixir/WORKFLOW.md`** after
  noting the mismatch in AGENTS.md change history or harness Notes—do not edit
  `elixir/WORKFLOW.md` from the harness agent.
