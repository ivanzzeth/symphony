...
- **Missing WORKFLOW.md**: Report and exit; cannot configure harness without the
  ...
  `elixir/WORKFLOW.md` (path passed to the running orchestrator). When Symphony
  injects a temp workflow path that is **gone or unreadable**, read
  **`elixir/WORKFLOW.md`** as the fallback contract **before** declaring a total
  blocker — only stop if **both** the injected path and the canonical repo file
  are unavailable.
