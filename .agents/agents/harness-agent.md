...
- **Missing or unreadable injected WORKFLOW path**: Try the monorepo canonical
  **`elixir/WORKFLOW.md`**. If that file is also missing or unreadable, report
  and exit—cannot configure harness without the Symphony execution contract.
  Never edit the workflow file from harness work.
