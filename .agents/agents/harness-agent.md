...
- **Missing or unreadable workflow path**: If the dispatch-injected path cannot
  be read, fall back to `elixir/WORKFLOW.md` in the repo workspace, document the
  fallback in AGENTS.md Change History, and continue. If that fallback is also
  missing or unreadable, report and exit; cannot configure harness without the
