...
9. **Injected workflow path fallback** — When the dispatch-provided workflow file path is missing or unreadable (common with ephemeral `/tmp/...` copies), read **`elixir/WORKFLOW.md`** as the contract source and proceed; record that fallback in **`AGENTS.md` Change History** when harness edits depended on it.
