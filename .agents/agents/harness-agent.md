...
- **Missing WORKFLOW.md**: Report and exit; cannot configure harness without the
  ...
  `elixir/WORKFLOW.md` (path passed to the running orchestrator). If the **injected**
  dispatch path (for example a temp workspace copy) is missing or unreadable,
  read `elixir/WORKFLOW.md` from the repo as fallback **read-only** and document
  which source was used—still never edit workflow files from the harness agent.
