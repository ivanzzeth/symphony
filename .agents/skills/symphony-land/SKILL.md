...
3. If uncommitted changes exist, run **`commit`** then **`push`** per WORKFLOW Step 2 (same flows as **`symphony-commit`** / **`symphony-push`** IDE-alias skills).
4. Check mergeability and conflicts against `base_branch`.
5. If conflicts: run **`pull`** to merge `origin/<base_branch>`, resolve, then **`push`** (equivalently **`symphony-pull`** / **`symphony-push`**).
...
10. After merge: transition the Linear issue to `Done` via the **`linear`** skill (**`symphony-linear`** is the same skill tree).
...
- Use **`commit`**, **`pull`**, and **`push`** for branch updates (`symphony-*` skill dirs are aliases). Call `gh pr merge` only once checks and review are green.
