...
2. Detect execution context:
   - **Symphony dispatch**: The harness run is driven by Symphony (for example `SymphonyElixir.Harness.Manager` after a `WORKFLOW.md` content-hash change, an explicit harness dispatch, or caller-supplied orchestrator context such as `_workspace/symphony_context.json`).
     1. **Hard prerequisite: read the workflow execution contract immediately.** The dispatch context usually provides a workflow file path — read that path in full first. If the path is **missing or unreadable** (for example a stale temp copy), fall back to the repo canonical contract at **`elixir/WORKFLOW.md`** in this monorepo — it must match what the running `symphony` process uses. Understand actual content, structure, and term usage; never guess.
     2. Only after a successful read (injected path or canonical fallback) may you proceed to audit `.agents/` and `AGENTS.md`.
...
2. **Fallback:** Injected workflow path from dispatch context is missing or unreadable,
   but repo **`elixir/WORKFLOW.md`** is readable. Expect harness to read the canonical
   contract and continue; record the fallback in **`AGENTS.md` Change History** when
   this path was used for the reconfiguration.
3. **Hard error:** No readable workflow contract at injected path **or**
   **`elixir/WORKFLOW.md`**. Expect harness to stop after reporting the blocker; no
   partial writes to skills.
4. **Continuation:** Resume harness configuration from current tree. Expect
   re-audit, no duplicate agent files, `linear` / `land` / `pull` alignment verified (including **`symphony-linear` / `symphony-land` / `symphony-pull`** mirror trees when present).
