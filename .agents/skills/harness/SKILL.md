...
2. Detect execution context:
   - **Symphony dispatch**: The harness run is driven by Symphony (for example `SymphonyElixir.Harness.Manager` after a `WORKFLOW.md` content-hash change, an explicit harness dispatch, or caller-supplied orchestrator context such as `_workspace/symphony_context.json`).
     ...
     2. **Injected path missing or unreadable:** If that path does not exist on disk (stale `/tmp/...` handle, different mount, or cleaned temp) or cannot be read, fall back to `elixir/WORKFLOW.md` at the workspace root for this monorepo—it is the same execution contract the orchestrator uses. Do **not** edit it; only read for audit. Note the fallback in AGENTS.md **Change History** when harness outputs change because of this pass.
     3. Only after a successful read (injected or fallback) may you proceed to audit `.agents/` and `AGENTS.md`.
...
- **Issue PR gates (checklist)**: `references/issue-execution-checklist.md` — maps to WORKFLOW **Step 1: Workpad bootstrap** (UI walkthrough, app-specific flow checks, ticket validation mirror, pull evidence in `Notes`) and **Step 2: Execute** (implement, `commit`, `push`, `land sweep`), plus Guardrails (no extra completion comments), `Todo`+PR flow, workpad edit fallback, PR feedback sweep, Manual QA Plan, mandatory validation gate, completion bar (runtime validation + media), blocked-access escape hatch, merge→Done (pointers only; `elixir/WORKFLOW.md` is authoritative).
