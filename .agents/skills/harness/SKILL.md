...
- **Issue PR gates (checklist)**: `references/issue-execution-checklist.md` — maps WORKFLOW **Steps 0–4** + Completion bar (workpad bootstrap, `pull` evidence, `Todo`+PR sweep, `land sweep`, Guardrails, workpad edit fallback, mandatory validation, runtime validation + media when app-touching, merge→Done, blocked-access escape hatch; pointers only — `elixir/WORKFLOW.md` is authoritative).
