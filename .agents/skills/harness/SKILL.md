...
5. **Symphony dispatch: read the execution contract FIRST, before changing anything.** The running `symphony` process injects a workflow file path into the dispatch context. Read that file in full when it exists and is readable; if it is missing or unreadable, read **`elixir/WORKFLOW.md`** (see `references/symphony-dispatch.md` → *Injected path resolution*). Never assume filenames or terminology without reading. Manual invocations still honor any workflow path mentioned in project context.
...
2. Detect execution context:
   - **Symphony dispatch**: The harness run is driven by Symphony (for example `SymphonyElixir.Harness.Manager` after a `WORKFLOW.md` content-hash change, an explicit harness dispatch, or caller-supplied orchestrator context such as `_workspace/symphony_context.json`).
     1. **Hard prerequisite: read the execution contract before edits.** Use the dispatch-injected workflow path when it exists and is readable; if it is missing or unreadable, read **`elixir/WORKFLOW.md`** (see `references/symphony-dispatch.md` → *Injected path resolution*). Understand YAML + Markdown content, structure, and term usage.
     2. Only after a successful read may you proceed to audit `.agents/` and `AGENTS.md`.
...
2. **Error:** Neither the injected workflow path nor fallback **`elixir/WORKFLOW.md`**
   is readable. Expect harness to stop after reporting the blocker; no partial
   writes to skills.
