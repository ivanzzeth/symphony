...
2. Detect execution context:
   - **Symphony dispatch**: The harness run is driven by Symphony (for example `SymphonyElixir.Harness.Manager` after a `WORKFLOW.md` content-hash change, an explicit harness dispatch, or caller-supplied orchestrator context such as `_workspace/symphony_context.json`).
     1. **Hard prerequisite: read the injected workflow file immediately when readable.** The dispatch context usually provides a workflow file path. Read that file in full before any other operation. If the path is missing or unreadable (ephemeral `/tmp/...` on another machine, deleted file, permissions), record that in the active issue **`## Codex Workpad`** **`Notes`** and read **`elixir/WORKFLOW.md`** as the canonical Symphony contract for this monorepo—**never** edit workflow contract files during harness work.
     2. Only after you have a complete picture from (1) may you proceed to audit `.agents/` and `AGENTS.md`.
