...
- The Markdown template is organized as **Step 0** (route), **Step 1** (workpad bootstrap), **Step 2** (execute), **Step 3** (`In Review`), **Step 4** (`Rework`), plus **Contract**, **Completion bar**, **Guardrails**, **Blocked-access escape hatch**, and **Prerequisite** — see `elixir/WORKFLOW.md` for exact wording.
...
- **`Todo` + attached PR at kickoff:** run the **full PR feedback sweep** after the workpad exists and **before** new feature work (WORKFLOW State → Skill routing + Step 2); before `In Review`, **`push`** required branch updates when fixes land.
