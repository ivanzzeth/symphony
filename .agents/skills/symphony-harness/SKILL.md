...
2. Detect execution context:
   - **Symphony dispatch**: The harness run is driven by Symphony (for example `SymphonyElixir.Harness.Manager` after a `WORKFLOW.md` content-hash change, an explicit harness dispatch, or caller-supplied orchestrator context such as `_workspace/symphony_context.json`).
     ...
     2. **Stale or missing injected path:** If that absolute path does not exist or cannot be read (expired temp workspace, different host, permissions), read **`elixir/WORKFLOW.md`** in this monorepo as the canonical Symphony execution contract. Record the fallback (intended path, actual path, reason) in AGENTS.md **Change History** and, when running under a Linear issue, in workpad **`Notes`**. Continue the harness audit—do not abort solely because a host-specific temp path disappeared when `elixir/WORKFLOW.md` is present.
...
4. **Stale injected path:** Dispatch supplies `/tmp/.../WORKFLOW.md` (or similar)
   but the file is gone. Expect fallback read of `elixir/WORKFLOW.md`, change-history
   note, then normal Phase 0 audit.
