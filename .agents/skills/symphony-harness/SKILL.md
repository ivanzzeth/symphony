...
2. Detect execution context:
   - **Symphony dispatch**: The harness run is driven by Symphony (for example `SymphonyElixir.Harness.Manager` after a `WORKFLOW.md` content-hash change, an explicit harness dispatch, or caller-supplied orchestrator context such as `_workspace/symphony_context.json`).
     ...
     2. If that path is missing, unreadable, or empty, fall back to this monorepo’s canonical **`elixir/WORKFLOW.md`** for harness alignment (read-only; **never** modify the contract from the harness agent).
     3. Only after a successful read (injected path or fallback) may you proceed to audit `.agents/` and `AGENTS.md`.
...
3. **Continuation:** Resume harness configuration from current tree. Expect
   re-audit, no duplicate agent files, **`pull`** / **`land`** / **`linear`** skill alignment verified (**`symphony-*`** mirror dirs, when present, must stay equivalent).
