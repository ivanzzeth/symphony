...
- The file has two layers: YAML config (tracker, polling, workspace, hooks, `agent` pool limits including **`stream_timeout_ms`**, `codex` runner) **and** the Markdown prompt after the second `---` (**Contract**, **State → Skill routing**, **Steps 0–4**, **Completion bar**, **Guardrails**, **Blocked-access escape hatch**, **Prerequisite**). Harness reconfiguration must keep **AGENTS.md** and issue-execution skills (`pull`, `push`, `land`, `linear`, …) aligned with **both** layers.
...
- **`Todo` + attached PR at kickoff:** run the **full PR feedback sweep** after the workpad exists and **before** new feature work (**State → Skill routing** / **Step 2 Execute** hygiene); **`push`** required branch updates before returning to **`In Review`** when fixes land.
...
- **Workpad edit fallback:** MCP comment update preferred; if unavailable use the **update script** path documented in the `linear` skill (**`symphony-linear`** is the same skill tree when present)—only block when both fail (WORKFLOW Guardrails).
