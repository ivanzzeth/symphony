...
- `pull`: use when push is rejected (non-fast-forward, stale branch).
- `commit`: use to create clean commits before pushing.
...
- Distinguish sync problems (use the **`pull`** skill) from auth/permissions problems (document fallbacks, then stop).
